...
public class PlayerTutorialScript : MonoBehaviour
{
    ...
    IEnumerator Interaction()
    {
        ...
        InteractableTutorial nearestInteractable = null;
        float nearestDistance = float.MaxValue;

        foreach (var hitCollider in hitColliders)
        {
            InteractableTutorial interactable = hitCollider.GetComponent<InteractableTutorial>();
            ...
            if (nearestInteractable != null && nearestInteractable.CanInteract && characterController.isGrounded)
            {
                ...
                nearestInteractable.InteractTutorial(this);
                canNoise = true;
                yield break;
            }
            else
